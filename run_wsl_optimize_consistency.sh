...
for NP in 1 4; do
  PREFIX="result/opt_consistency_${NX}_${NY}_${NZ}_np${NP}"
...
np1 = base / f"opt_consistency_{nx}_{ny}_{nz}_np1_opt_summary.txt"
np4 = base / f"opt_consistency_{nx}_{ny}_{nz}_np4_opt_summary.txt"
c1 = read_value(np1, "final_compliance")
c4 = read_value(np4, "final_compliance")
v1 = read_value(np1, "final_volume")
v4 = read_value(np4, "final_volume")
crel = abs(c1 - c4) / max(abs(c1), 1.0)
vrel = abs(v1 - v4) / max(abs(v1), 1.0)
print(f"np1 final_compliance={c1:.12e}, final_volume={v1:.12e}")
print(f"np4 final_compliance={c4:.12e}, final_volume={v4:.12e}")
print(f"compliance_relative_difference={crel:.12e}")
print(f"volume_relative_difference={vrel:.12e}")
if crel > 1e-8 or vrel > 1e-8:
    raise SystemExit("Optimization rank consistency check failed")
PY
